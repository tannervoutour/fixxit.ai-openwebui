				...
				{#if filteredSettings.length > 0}
					{#each filteredSettings as tabId (tabId)}
						...
						{:else if tabId === 'connections'}
							{#if $user?.role === 'admin' || (($user?.role === 'user' || $user?.role === 'manager') && $config?.features?.enable_direct_connections)}
								<button
									role="tab"
							...
							{/if}
						{:else if tabId === 'tools'}
							{#if $user?.role === 'admin' || (($user?.role === 'user' || $user?.role === 'manager') && $user?.permissions?.features?.direct_tool_servers)}
								<button
									role="tab"
