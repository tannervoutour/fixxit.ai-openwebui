...
export const getGroupDatabaseConfig = async (token: string, groupId: string) => {
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/groups/id/${groupId}/database`, {
		method: 'GET',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
...
export const testDatabaseConnection = async (token: string, config: object) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/groups/database/test`, {
		method: 'POST',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
